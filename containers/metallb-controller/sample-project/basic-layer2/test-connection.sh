...
# MetalLB Basic Layer 2 Test Script
# This script tests the LoadBalancer service setup

set -e

echo "🔍 Testing MetalLB Basic Layer 2 Setup"
echo "======================================"
...
if ! kubectl get ipaddresspool first-pool -n metallb-system &> /dev/null; then
    print_error "IP address pool 'first-pool' not found"
...
print_status "Checking L2 advertisement..."
if ! kubectl get l2advertisement example -n metallb-system &> /dev/null; then
    print_error "L2 advertisement 'example' not found"
    print_status "Applying MetalLB configuration..."
    kubectl apply -f metallb-config.yaml
    sleep 5
fi

print_success "L2 advertisement is configured"
...
if ! kubectl get service nginx-service &> /dev/null; then
    print_error "LoadBalancer service 'nginx-service' not found"
    exit 1
fi
...
echo ""
print_success "MetalLB Basic Layer 2 setup is working correctly!"
...
echo "kubectl delete -f metallb-config.yaml"
